...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'POST' && action === 'register') {
      ...
      await sql`INSERT INTO users (id, email, password_hash, display_name) VALUES (${userId}, ${email}, ${passwordHash}, ${name})`;
      ...
      return res.status(201).json({ success: true, token, user: { id: userId, email, displayName: name, avatarUrl: '', bio: '' } });
    }
    ...
    if (req.method === 'POST' && action === 'login') {
      ...
      return res.status(200).json({ success: true, token, user: { id: user.id, email: user.email, displayName: user.display_name, avatarUrl: user.avatar_url, bio: user.bio } });
    }
    ...
    if (req.method === 'GET' && action === 'profile') {
      ...
      const users = await sql`SELECT id, email, display_name, avatar_url, bio, created_at FROM users WHERE id = ${authUser.userId}`;
      if (users.length === 0) return res.status(404).json({ error: 'User not found' });
      const u = users[0];
      return res.status(200).json({ id: u.id, email: u.email, displayName: u.display_name, avatarUrl: u.avatar_url, bio: u.bio, createdAt: u.created_at });
    }
