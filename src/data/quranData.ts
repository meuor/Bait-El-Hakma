...
export const SURAH_LIST: SurahInfo[] = [
  ...
  { number: 37, name: "الصافات", englishName: "As-Saffat", englishNameTranslation: "Those Ranks", numberOfAyahs: 182, revelationType: "Meccan" },
...
export function getDailyPortion(dayOfMonth: number): { startSurah: number; startAyah: number; endSurah: number; endAyah: number } {
  ...
  const endAyahOffset = startAyahOffset + dailyAyahs - 1;

  let accumulated = 0;
  let startSurah = 1, startAyahInSurah = 1;
  let endSurah = 1, endAyahInSurah = 1;

  for (const surah of SURAH_LIST) {
    const surahEnd = accumulated + surah.numberOfAyahs;
    if (surahEnd > startAyahOffset && accumulated <= startAyahOffset) {
      startSurah = surah.number;
      startAyahInSurah = startAyahOffset - accumulated + 1;
    }
    if (surahEnd > endAyahOffset && accumulated <= endAyahOffset) {
      endSurah = surah.number;
      endAyahInSurah = endAyahOffset - accumulated + 1;
      break;
    }
    accumulated = surahEnd;
  }

  return { startSurah, startAyah: startAyahInSurah, endSurah, endAyah: endAyahInSurah };
}
